...
class AuthHandler (object):
    ...
    def _request_auth(self):
        m = Message()
        m.add_byte(pack_byte(MSG_SERVICE_REQUEST))
        m.add_string(b'ssh-userauth')
        self.transport._send_message(m)

    def _disconnect_service_not_available(self):
        m = Message()
        m.add_byte(pack_byte(MSG_DISCONNECT))
    ...
    def _get_session_blob(self, key, service, username):
        ...
        m.add_byte(pack_byte(MSG_USERAUTH_REQUEST))
    ...
    def _parse_service_request(self, m):
        ...
        if self.transport.server_mode and (service == b'ssh-userauth'):
            # accepted
            m = Message()
            m.add_byte(pack_byte(MSG_SERVICE_ACCEPT))
            m.add_string(service)
            self.transport._send_message(m)
            return
        # dunno this one
    ...
    def _send_auth_result(self, username, method, result):
        ...
        if result == AUTH_SUCCESSFUL:
            ...
            m.add_byte(pack_byte(MSG_USERAUTH_SUCCESS))
            self.authenticated = True
        else:
            self.transport._log(INFO, 'Auth rejected (%s).' % method)
            m.add_byte(pack_byte(MSG_USERAUTH_FAILURE))
    ...
    def _interactive_query(self, q):
        ...
        m = Message()
        m.add_byte(pack_byte(MSG_USERAUTH_INFO_REQUEST))
        m.add_string(q.name)
        ...
        if method == b'none':
            ...
            if result != AUTH_FAILED:
                ...
                if not sig_attached:
                    ...
                    m = Message()
                    m.add_byte(pack_byte(MSG_USERAUTH_PK_OK))
    ...
    def _parse_userauth_info_request(self, m):
        ...
        m = Message()
        m.add_byte(pack_byte(MSG_USERAUTH_INFO_RESPONSE))
